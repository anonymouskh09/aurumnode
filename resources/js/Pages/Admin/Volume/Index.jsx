                ...
                {user && (
                    <>
                        <div className="p-4 border rounded-xl bg-white">
                            ...
                            <p><strong>Left volume (USDT):</strong> ${Number(user.left_points_total ?? 0).toFixed(2)} | <strong>Right volume (USDT):</strong> ${Number(user.right_points_total ?? 0).toFixed(2)}</p>
                        </div>
                        <div className="p-4 border rounded-xl bg-white">
                            <h3 className="font-semibold mb-2">Add volume</h3>
                            <form onSubmit={(e) => { e.preventDefault(); router.post('/admin/volume/add', { ...addForm.data, user_id: user.id, points: addForm.data.amount_usdt }); }} className="space-y-2 max-w-sm">
                                ...
                                <input type="number" step="0.01" min="0.01" placeholder="Amount (USDT)" value={addForm.data.amount_usdt} onChange={(e) => addForm.setData('amount_usdt', e.target.value)} className="w-full rounded border px-3 py-2" required />
